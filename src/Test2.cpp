    ...
    void createCommandBuffers()
    {
        ...
        SwapChainSupportDetails querySwapChainSupport(VkPhysicalDevice d)
        {
            ...
            if (formatCount > 0) {
                ...
                vkGetPhysicalDeviceSurfaceFormatsKHR(
                    d, surface, &formatCount, details.formats.data());
            }
            ...
            const VkDebugUtilsMessengerCallbackDataEXT* pCallbackData, void*)
        {
            if (messageSeverity == VK_DEBUG_UTILS_MESSAGE_SEVERITY_VERBOSE_BIT_EXT) {
                ...
                spdlog::warn(
                    "Validation unknown: {} - {}", messageSeverity, pCallbackData->pMessage);
            }
            return VK_FALSE;
        }
        ...
        void createInstance()
        {
            ...
            for (const auto& requiredExt : requiredExtensions) {
                ...
                if (!std::any_of(extensions.begin(), extensions.end(), [&](const auto& ext) {
                        return std::string(ext.extensionName) == extName;
                    })) {
